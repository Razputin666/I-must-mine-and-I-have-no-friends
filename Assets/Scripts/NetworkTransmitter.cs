...
public class NetworkTransmitter : NetworkBehaviour
{
    ...
    public const int RELIABLE_SEQUENCED_CHANNEL = Channels.DefaultReliable;
    ...
    public event UnityAction<int, byte[]> OnDataCompletelySent;
    ...
    public IEnumerator SendBytesToClientsRoutine(int transmissionId, byte[] data)
    {
        ...
        if (null != OnDataCompletelySent)
            OnDataCompletelySent.Invoke(transmissionId, dataToTransmit.data);
    }
    ...
    private void RpcReceiveBytes(int transmissionId, byte[] recBuffer)
    {
        ...
        Debug.Log(dataToReceive.curDataIndex + " " + dataToReceive.data.Length);
        ...
        Debug.Log(OnDataCompletelyReceived);
        if (null != OnDataCompletelyReceived)
            OnDataCompletelyReceived.Invoke(transmissionId, dataToReceive.data);
    }

    [Server]
    public void SendBytesToClient(NetworkConnection conn, int transmissionId, byte[] data)
    {
        Debug.Assert(!serverTransmissionIds.Contains(transmissionId));
        StartCoroutine(SendBytesToClientRoutine(conn, transmissionId, data));
    }
#endregion

    #region SingleSend
    [Server]
    public IEnumerator SendBytesToClientRoutine(NetworkConnection conn, int transmissionId, byte[] data)
    {
        Debug.Assert(!serverTransmissionIds.Contains(transmissionId));
        Debug.Log(LOG_PREFIX + "SendBytesToClients processId=" + transmissionId + " | datasize=" + data.Length);

        //tell client that he is going to receive some data and tell him how much it will be.
        RpcTargetPrepareToReceiveBytes(conn, transmissionId, data.Length);
        yield return null;
        ...
        while (dataToTransmit.curDataIndex < dataToTransmit.data.Length - 1)
        {
            ...
            //send the chunk
            RpcTargetReceiveBytes(conn, transmissionId, buffer);
            dataToTransmit.curDataIndex += bufferSize;
    ...
    [TargetRpc]
    private void RpcTargetPrepareToReceiveBytes(NetworkConnection conn, int transmissionId, int expectedSize)
    {
        if (clientTransmissionData.ContainsKey(transmissionId))
            return;
    ...
    //use reliable sequenced channel to ensure bytes are sent in correct order
    [TargetRpc(channel = RELIABLE_SEQUENCED_CHANNEL)]
    private void RpcTargetReceiveBytes(NetworkConnection conn, int transmissionId, byte[] recBuffer)
    {
        //already completely received or not prepared?
        if (!clientTransmissionData.ContainsKey(transmissionId))
            return;
