...
public class TerrainGenerator
{
    ...
    public TILE_TYPES[,] generatedTiles;
    ...
    {
        generatedTiles = new TILE_TYPES[width, height];

        for (int px = 0; px < generatedTiles.GetLength(0); px++)
        {
            ...
            for (int py = 0; py < generatedTiles.GetLength(1); py++)
            {
                if (py < stone)
                {
                    generatedTiles[px, py] = TILE_TYPES.STONE;
                    //The next three lines make dirt spots in random places
                    if (NoiseInt(px, py, 12, 16, 1) > 10)
                    {  //dirt spots
                        generatedTiles[px, py] = TILE_TYPES.DIRT;
                    }
                    //The next three lines remove dirt and rock to make caves in certain places
                    if (NoiseInt(px, py * 2, 16, 14, 1) > 10)
                    { //Caves
                        generatedTiles[px, py] = TILE_TYPES.NONE;
                    }
                }
                else if (py < dirt)
                {
                    generatedTiles[px, py] = TILE_TYPES.GRASS;
                }
            }
        }
    }
    public TILE_TYPES[,] GetGeneratedTiles()
    {
